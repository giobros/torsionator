    ...
    def run(
        ...
        if net_charge != 0:
            ...
            if charge_incompatible:
                msg = (
                    f"ERROR: Method(s) {charge_incompatible} do not support charged molecules "
        ...
        if spin != 1:
            ...
            if spin_incompatible:
                msg = (
                    f"ERROR: Method(s) {spin_incompatible} do not support non-singlet spin states "
        ...
        is_all = dihedral.strip().lower() == "all"
        ...
        for m in methods:
            ...
            if is_all:
                # ── Phase 1: NN scan + GAFF2/old ──────────────────────────────
                # Antechamber runs once (first dihedral); topology + mol2/frcmod
                # are copied to all others.
                ref_d = dihedrals_to_scan[0]
                for i, d_tup in enumerate(dihedrals_to_scan):
                    ...
                    io_utils.run_gaff2_old(
                        ...
                        postscan_make_expanded_for_mdgx=False,
                        MCS=False, mcs_xyz=None, step_size=step_size,
                        net_charge=net_charge, spin=spin,
                        skip_antechamber=(i > 0),
                    )
                    if i == 0:
                        # Propagate shared antechamber + topology to all others
                        for dst_d in dihedrals_to_scan[1:]:
                            io_utils._copy_antechamber_and_leap_to_dihedral(
                                self.cfg, self.log, m, ref_d, dst_d
                            )

                # ── Phase 2: detect basis per dihedral ────────────────────────
                if auto_basis:
                    basis_per_dihedral = {
                        d_tup: io_utils.detect_basis_from_antechamber(self.log, self.cfg, d_tup, m)
                        for d_tup in dihedrals_to_scan
                    }
                else:
                    basis_per_dihedral = {d_tup: basis_tuple for d_tup in dihedrals_to_scan}

                # ── Phase 3: joint mdgx (one run per hrst for all dihedrals) ──
                shared_param_results = io_utils.run_parametrization_all(
                    self.log, self.cfg, m, dihedrals_to_scan,
                    MCS=False, basis_per_dihedral=basis_per_dihedral, expand_basis=expand_basis,
                )

                # ── Phase 4: GAFF2/new sander scan per dihedral ───────────────
                for d_tup in dihedrals_to_scan:
                    pdb_for_scan = best_conf[m].get(d_tup, minimized[m]) if use_conformers else minimized[m]
                    io_utils.run_gaff2_new(
                        self.log, self.cfg, root_name, m, d_tup, pdb_for_scan,
                        shared_param_results, MCS=False, mcs_xyz=None, step_size=step_size,
                        net_charge=net_charge, spin=spin,
                    )

                # ── Phase 5: minimax best hrst + frcmod + plots ───────────────
                kcal_profiles = io_utils.select_and_update_frcmod_all(
                    self.log, self.cfg, root_name, m, dihedrals_to_scan,
                    self.final_frcmod, shared_param_results, tag="", MCS=False,
                    expand_basis=expand_basis,
                )
                for d_tup in dihedrals_to_scan:
                    try:
                        plot_shifted_energy_profiles(self.cfg.base_dir, d_tup, m, kcal_profiles[d_tup])
    ...
    def _run_mcs(
        ...
        *,
        ...
        for m in methods:
            ...
            if is_all:
                # Collect mcs_xyz per dihedral first
                mcs_xyz_per_dihedral: Dict[Tuple, str] = {}
                for d_tup in dihedrals:
                    out_dir = _MCS_merge_scans(
                        self, self.cfg.base_dir, d_tup, m,
                        scanned_tags[m].get(d_tup, []),
                        step_size=step_size, mode="absolute",
                    )
                ...
                ref_d = dihedrals[0]
                for i, d_tup in enumerate(dihedrals):
                    io_utils.run_gaff2_old(
                        self.log, self.cfg, root_name, m, d_tup, ref_pdb,
                        basis_periodicities=basis_tuple or (1,),
                        postscan_make_expanded_for_mdgx=False,
                        MCS=True, mcs_xyz=mcs_xyz_per_dihedral[d_tup], step_size=step_size,
                        net_charge=net_charge, spin=spin,
                        skip_antechamber=(i > 0),
                    )
                    if i == 0:
                        for dst_d in dihedrals[1:]:
                            io_utils._copy_antechamber_and_leap_to_dihedral(
                                self.cfg, self.log, m, ref_d, dst_d
                            )
                ...
                if auto_basis:
                    ...
                    basis_per_dihedral = {d_tup: basis_tuple for d_tup in dihedrals}

                # ── Phase 3: joint mdgx ────────────────────────────────────────
                shared_param_results = io_utils.run_parametrization_all(
                    self.log, self.cfg, m, dihedrals,
                    MCS=True, basis_per_dihedral=basis_per_dihedral, expand_basis=expand_basis,
                )

                # ── Phase 4: GAFF2/new per dihedral ───────────────────────────
                for d_tup in dihedrals:
                    io_utils.run_gaff2_new(
                        self.log, self.cfg, root_name, m, d_tup, ref_pdb,
                        shared_param_results, MCS=True,
                        mcs_xyz=mcs_xyz_per_dihedral[d_tup], step_size=step_size,
                        net_charge=net_charge, spin=spin,
                    )

                # ── Phase 5: minimax best hrst + frcmod + plots ───────────────
                kcal_profiles = io_utils.select_and_update_frcmod_all(
                    self.log, self.cfg, root_name, m, dihedrals,
                    self.final_frcmod, shared_param_results, tag="_MCS", MCS=True,
                    expand_basis=expand_basis,
                )
                for d_tup in dihedrals:
                    try:
                        plot_shifted_energy_profiles_mcs(
                            self.cfg.base_dir, d_tup, m, best_profile=kcal_profiles[d_tup],
                        )
                    except Exception as exc:
