...
class Workflow:
    ...
    def run(
        ...
        # obi and mace do not natively support charged molecules:
        # the NNP calculation will proceed but ignore the charge.
        # The net_charge is still forwarded to GAFF2/antechamber.
        if net_charge != 0:
            charge_incompatible = [m for m in methods if m in ("obi", "mace")]
            if charge_incompatible:
                self.log.warning(
                    "Method(s) %s do not natively support charged molecules "
                    "(net_charge=%d). The NNP energy/forces will be computed as if the "
                    "molecule is neutral. The charge IS forwarded to GAFF2/antechamber. "
                    "Consider using --method uma for charged systems.",
                    charge_incompatible, net_charge,
                )
        ...
        if spin != 1:
            ...
            if spin_incompatible:
                self.log.warning(
                    "Method(s) %s do not natively support non-singlet spin states "
                    "(spin=%d). The NNP energy/forces will be computed as if spin=1. "
                    "The spin IS forwarded to GAFF2/antechamber. "
                    "Consider using --method uma for open-shell systems.",
                    spin_incompatible, spin,
                )
