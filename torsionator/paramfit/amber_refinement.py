...
Public API:
  ...
  - find_best_hrst_multi_dihedral
  - get_atom_types_from_mol2
  - param_input_file
  - param_input_file_multi
  - update_frcmod
...
def find_best_hrst_multi_dihedral(
    nn_energy_files: Dict[tuple, str],
    gaff2_new_dirs: Dict[tuple, str],
    log,
) -> Tuple[float, str, Dict[tuple, float], Dict[tuple, str]]:
    """
    Select best hrst with minimax: argmin_hrst(max_i RMSE_i).
    Returns (best_hrst, best_frcmod_path, {d_tup: rmse}, {d_tup: kcal_profile}).
    """
    dihedrals = list(nn_energy_files.keys())
    first_d = dihedrals[0]

    hrst_vals: List[float] = []
    for item in os.listdir(gaff2_new_dirs[first_d]):
        if os.path.isdir(os.path.join(gaff2_new_dirs[first_d], item)):
            try:
                hrst_vals.append(float(item))
            except ValueError:
                continue
    if not hrst_vals:
        raise FileNotFoundError(f"No hrst directories in {gaff2_new_dirs[first_d]}")

    scores: Dict[float, float] = {}
    all_rmse: Dict[float, Dict[tuple, float]] = {}

    for hrst_val in sorted(hrst_vals):
        rmses: Dict[tuple, float] = {}
        for d_tup in dihedrals:
            hartree_file = os.path.join(gaff2_new_dirs[d_tup], str(hrst_val), "dd_ee_shifted_hartree")
            if not os.path.exists(hartree_file):
                log.warning("[RMSD-MULTI] missing %s", hartree_file)
                rmses[d_tup] = float("inf")
                continue
            nn_angles, nn_energies = load_energy_file(nn_energy_files[d_tup])
            mm_angles, mm_energies = load_energy_file(hartree_file)
            if len(mm_energies) != len(nn_energies):
                mm_energies = np.interp(nn_angles, mm_angles, mm_energies)
            rmses[d_tup] = calculate_rmsd(nn_energies, mm_energies)
        scores[hrst_val] = max(rmses.values())
        all_rmse[hrst_val] = rmses

    best_hrst = min(scores, key=scores.__getitem__)
    log.info("[RMSD-MULTI] BEST hrst=%s  max_RMSE=%.6f", best_hrst, scores[best_hrst])
    for d_tup, rmse in all_rmse[best_hrst].items():
        log.info("[RMSD-MULTI]   dihedral=%s  RMSE=%.6f", d_tup, rmse)

    best_frcmod = os.path.join(gaff2_new_dirs[first_d], str(best_hrst), "antechamber", "MOL.frcmod")
    kcal_profiles = {
        d_tup: os.path.join(gaff2_new_dirs[d_tup], str(best_hrst), "dd_ee_shifted")
        for d_tup in dihedrals
    }
    return best_hrst, best_frcmod, all_rmse[best_hrst], kcal_profiles
...
def param_input_file_multi(
    dihedral_entries: List[Dict],
    method: str,
    hrst_val: float,
    output_dir: str,
    top_path: str,
    ...
    """
    Create a single mdgx .in file that fits ALL dihedrals simultaneously.
    dihedral_entries: list of {d_tup, atom_types, scan_dir, method_subdir}.
    Returns (pfile, dat_path, out_path).
    """
    os.makedirs(output_dir, exist_ok=True)
    top_abs = os.path.abspath(top_path)
    frcmod_abs = os.path.abspath(frcmod_path)
    out_dat = os.path.join(output_dir, f"{hrst_val}.dat")
    out_out = os.path.join(output_dir, f"{hrst_val}.out")
    pfile = os.path.join(output_dir, f"parametrization_{method}_all.in")

    with open(pfile, "w") as f:
        f.write("&files\n")
        f.write(f"  -p      {top_abs}\n")
        f.write(f"  -parm   {frcmod_abs}\n")
        f.write(f"  -d      {os.path.abspath(out_dat)}\n")
        f.write(f"  -o      {os.path.abspath(out_out)}\n")
        f.write("&end\n\n&param\n")
        for entry in dihedral_entries:
            traj = os.path.abspath(
                os.path.join(entry["scan_dir"], entry["method_subdir"], "geometries.nc")
            )
            energy = os.path.abspath(
                os.path.join(entry["scan_dir"], entry["method_subdir"], "energies.dat")
            )
            f.write(f"System {top_abs} {traj} {energy}\n")
        f.write("\n  ParmOutput frcmod\n")
        f.write("  eunits    hartree,\n\n")
        f.write("  verbose    1,\n\n")
        f.write("  % Torsion fitting input\n")
        seen_fith: set = set()
        for entry in dihedral_entries:
            dih_types = " ".join(entry["atom_types"][i] for i in entry["d_tup"])
            if dih_types not in seen_fith:
                f.write(f"  fith {dih_types}\n")
                seen_fith.add(dih_types)
        f.write(f"  hrst       {hrst_val},\n&end\n")
...
def update_frcmod(
    ...
    # Sort each consecutive dihedral group from most-negative period to +1 last.
    # In AMBER frcmod format negative period means "more terms follow"; only the
    # last line in a group may have positive period.  _update_key appends new
    # terms after the existing positive-period term, so the group ends up with a
    # negative period as its final entry, causing tleap to skip whatever dihedral
    # follows.  Re-sorting each run fixes the ordering (e.g. -6,-4,-3,-2,+1).
    i = 0
    while i < len(body):
        pr = parsed_body[i]
        if not pr:
        ...
        run = [i]
        j = i + 1
        while j < len(body) and parsed_body[j] and str(parsed_body[j]["key"]) == key:
            run.append(j)
            j += 1
        if len(run) > 1:
            def _sort_key(bi: int) -> float:
                p = parsed_body[bi]
                per = float(p["periodicity"]) if p else 0.0
                # positive period sorts last (large value); negative sorts by abs descending
                return 1e9 if per > 0 else per
            sorted_run = sorted(run, key=_sort_key)
            if sorted_run != run:
                saved = [body[bi] for bi in sorted_run]
                saved_parsed = [parsed_body[bi] for bi in sorted_run]
                for new_pos, bi in enumerate(run):
                    body[bi] = saved[new_pos]
                    parsed_body[bi] = saved_parsed[new_pos]
        i = j
