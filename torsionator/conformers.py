...
class ConformerScreen:
    ...
    def screen_by_rmsd(self, pdb_dir: str, threshold: float) -> None:
        """Remove conformers that are within `threshold` RMSD of any already-kept one. 0.pdb is never screened."""
        all_files = sorted(
            [os.path.join(pdb_dir, f) for f in os.listdir(pdb_dir) if f.endswith(".pdb")],
            key=_int_stem_key,
        )
        files = [f for f in all_files if Path(f).stem != "0"]
        if not files:
            return
        kept = [files[0]]
        for cand in files[1:]:
    ...
    def ensure_ref_if_unique(self, conformers_dir: str, reference_pdb: str, threshold: float) -> None:
        """Always copy the reference as 0.pdb."""
